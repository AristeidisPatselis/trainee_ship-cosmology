...
z_data = np.loadtxt(find_data_file("d_z_vals.txt"))
H_data = np.loadtxt(find_data_file("d_H_vals.txt"))
sigma_data = np.loadtxt(find_data_file("d_sigma_vals.txt"))
N_DATA = len(z_data)
print(f"Loaded {N_DATA} DESI data points.")
...
if __name__ == "__main__":
    ...
    # --- MODEL I FIT ---
    out1 = os.path.join(HERE, "outputs_horndeski_desi_model1")
    ...
    if len(H1_samples) > 0:
        ...
        ax1.errorbar(z_data, H_data, yerr=sigma_data, fmt="o", color="black", ecolor="gray", capsize=2, label="DESI Data")
    ...
    # --- MODEL II FIT ---
    out2 = os.path.join(HERE, "outputs_horndeski_desi_model2")
    ...
    if len(H2_samples) > 0:
        ...
        ax2.errorbar(z_data, H_data, yerr=sigma_data, fmt="o", color="black", ecolor="gray", capsize=2, label="DESI Data")
    ...
    out_comp = os.path.join(HERE, "outputs_horndeski_desi_comparison")
    os.makedirs(out_comp, exist_ok=True)

    fig_comp, ax_c = plt.subplots(figsize=(8, 5.5))
    ax_c.errorbar(z_data, H_data, yerr=sigma_data, fmt="o", ms=4, color="black", ecolor="gray", elinewidth=1, capsize=2, label="DESI Data")
    H_lcdm_ref = 68.0 * np.sqrt(0.3 * (1+z_grid)**3 + 0.7)
    ...
    ax_c.set_title(r"DESI: Full Horndeski Models vs. $\Lambda$CDM", fontsize=12)
    ...
    out_stab = os.path.join(HERE, "outputs_horndeski_desi_stability")
