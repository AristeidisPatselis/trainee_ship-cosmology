...
if __name__ == "__main__":
    ...
    # --- MODEL I FIT ---
    out1 = os.path.join(HERE, "outputs_horndeski_cc_model1")
    ...
    # --- MODEL II FIT ---
    out2 = os.path.join(HERE, "outputs_horndeski_cc_model2")
    ...
    out_comp = os.path.join(HERE, "outputs_horndeski_cc_comparison")
    os.makedirs(out_comp, exist_ok=True)

    fig_comp, ax_c = plt.subplots(figsize=(8, 5.5))
    ax_c.errorbar(z_data, H_data, yerr=sigma_data, fmt="o", ms=4, color="black", ecolor="gray", elinewidth=1, capsize=2, label="CC Data")
    H_lcdm_ref = 68.0 * np.sqrt(0.3 * (1+z_grid)**3 + 0.7)
    ...
    ax_c.set_title(r"Cosmic Chronometers: Full Horndeski Models vs. $\Lambda$CDM", fontsize=12)
    ...
    out_stab = os.path.join(HERE, "outputs_horndeski_cc_stability")
