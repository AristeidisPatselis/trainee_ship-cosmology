...
z_data = np.loadtxt(find_data_file("co_z_vals.txt"))
H_data = np.loadtxt(find_data_file("co_H_vals.txt"))
sigma_data = np.loadtxt(find_data_file("co_sigma_vals.txt"))
N_DATA = len(z_data)
print(f"Loaded {N_DATA} Combined data points.")
...
if __name__ == "__main__":
    ...
    # --- MODEL I FIT ---
    out1 = os.path.join(HERE, "outputs_horndeski_combined_model1")
    ...
    if len(H1_samples) > 0:
        ...
        ax1.errorbar(z_data, H_data, yerr=sigma_data, fmt="o", color="black", ecolor="gray", capsize=2, label="Combined Data")
    ...
    # --- MODEL II FIT ---
    out2 = os.path.join(HERE, "outputs_horndeski_combined_model2")
    ...
    if len(H2_samples) > 0:
        ...
        ax2.errorbar(z_data, H_data, yerr=sigma_data, fmt="o", color="black", ecolor="gray", capsize=2, label="Combined Data")
    ...
    out_comp = os.path.join(HERE, "outputs_horndeski_combined_comparison")
    os.makedirs(out_comp, exist_ok=True)

    fig_comp, ax_c = plt.subplots(figsize=(8, 5.5))
    ax_c.errorbar(z_data, H_data, yerr=sigma_data, fmt="o", ms=4, color="black", ecolor="gray", elinewidth=1, capsize=2, label="Combined Data")
    H_lcdm_ref = 68.0 * np.sqrt(0.3 * (1+z_grid)**3 + 0.7)
    ...
    ax_c.set_title(r"Combined: Full Horndeski Models vs. $\Lambda$CDM", fontsize=12)
    ...
    out_stab = os.path.join(HERE, "outputs_horndeski_combined_stability")
    os.makedirs(out_stab, exist_ok=True)

    z_stab = np.linspace(0, 5, 100)
