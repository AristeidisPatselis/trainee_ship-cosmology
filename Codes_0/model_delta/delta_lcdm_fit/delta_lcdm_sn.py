...
the data prefer a dark-energy term whose effective density scales with H
itself, rather than staying a pure cosmological constant.

This is the SN analogue of delta_lcdm_fit.py (which fits cosmic-chronometer
H(z) data). The underlying algebraic model is identical; the observable is
now the distance modulus

    mu(z) = 5 log10(d_L(z)/10 pc),
    d_L(z) = (1+z) * c * Integral_0^z dz'/H(z'),

so every likelihood evaluation requires solving the implicit H equation on a
redshift grid and then numerically integrating 1/H.

Parameter order is (H0, Om, delta) throughout, matching the chronometer
script, so the two can be compared line-by-line.
"""
...
CONTOUR_GRID = 40          # now cheap: batched vectorized solve, not per-point brentq
PROFILE_POINTS = 40
...
NEWTON_ITERS = 60          # iterations for the vectorized implicit H(z) solve
N_MULTISTART = 6
...
def setup_matplotlib(use_latex=False):
    ...
    hundreds of subprocess launches per figure, which shows up as
    system-wide lag (process spawn + disk I/O), not just CPU load. Leave
    this False unless you specifically need LaTeX-rendered labels; mathtext
    covers everything used in this script (\\delta, \\Omega_{m,0}, etc.).
    """
    if use_latex:
        try:
            rc('text', usetex=True)
...
def find_file_recursively(filename, data_dir, max_depth=4):
    ...
    misconfigured DATA_DIR (pointing somewhere broad, or containing a
    symlink loop) can't turn into an unbounded whole-disk scan, which shows
    up as system-wide I/O lag rather than a normal, contained slowdown.
    """
...
# =============================================================================
# 2. MODEL: implicit H(z) via vectorized Newton, then distance modulus
# =============================================================================

def H_lcdm(z, H0, Om):
    """Standard flat LambdaCDM (closed form)."""
    return H0 * np.sqrt(Om * (1 + z) ** 3 + (1 - Om))


def _solve_H_newton(z, H0, Om, delta, n_iter=NEWTON_ITERS):
    """
    Vectorized solve of
        H - H0*sqrt(Om*(1+z)^3 + (1-Om)*(H/H0)^delta) = 0
    for H, via Newton iteration.

    z, H0, Om, delta must be broadcastable against each other (numpy rules).
    Typical uses:
      - single parameter set, many redshifts: z shape (nz,), H0/Om/delta scalars.
      - a whole parameter grid at once: z shape (1, nz), H0/Om/delta shape
        (n_params, 1) -> output shape (n_params, nz).

    This replaces the old per-point scipy.brentq loop; it is exact in the
    delta=0 limit (handled analytically) and converges in a fixed, small
    number of iterations for the delta range used here.
    """
    ...
    one_plus_z3 = (1.0 + z) ** 3
    lcdm_inside = Om * one_plus_z3 + (1.0 - Om)

    near_zero = np.abs(delta) < 1e-12
    delta_safe = np.where(near_zero, 0.0, delta)

    # Initial guess: pure LambdaCDM value (exact when delta = 0)
    H = H0 * np.sqrt(np.clip(lcdm_inside, 1e-12, None))
    H, H0b, Omb, one_plus_z3b = np.broadcast_arrays(H, H0, Om, one_plus_z3)
    H = H.copy()

    for _ in range(n_iter):
        ratio = np.clip(H / H0b, 1e-8, None)
        pow_term = np.where(near_zero, 1.0, ratio ** delta_safe)
        inside = np.clip(Omb * one_plus_z3b + (1.0 - Omb) * pow_term, 1e-12, None)
        sq = np.sqrt(inside)
        g = H - H0b * sq

        dpow = np.where(near_zero, 0.0, delta_safe * ratio ** (delta_safe - 1.0))
        dinside_dH = (1.0 - Omb) * dpow / H0b
        dg = 1.0 - H0b * dinside_dH / (2.0 * sq)
        dg = np.where(np.abs(dg) < 1e-10, 1e-10, dg)

        H = np.clip(H - g / dg, 1e-6 * H0b, 1e6 * H0b)

    return H


def model_H(z_eval, H0, Om, delta):
    """Solve for H at every redshift in z_eval, for a single parameter set."""
    z_eval = np.atleast_1d(np.asarray(z_eval, dtype=float))
    if abs(delta) < 1e-12:
        return H_lcdm(z_eval, H0, Om)
    return _solve_H_newton(z_eval, H0, Om, delta)
...
def mu_model(z, H0, Om, delta, z_grid_points=Z_GRID_POINTS):
    """
    Distance modulus for the delta-LCDM model at a single (H0, Om, delta).
    """
    z = np.atleast_1d(np.asarray(z, dtype=float))
    z_grid = _get_integration_grid(z, z_grid_points)

    H_grid = model_H(z_grid, H0, Om, delta)
...
def mu_model_batch(z_obs, H0_arr, Om_arr, delta_arr, z_grid_points=Z_GRID_POINTS):
    """
    Distance modulus for a whole BATCH of (H0, Om, delta) parameter sets at
    ...
    This is what makes the contour/grid scans fast: instead of looping over
    every grid point and re-solving H(z) with brentq, all parameter sets are
    solved simultaneously via broadcasting in _solve_H_newton.
    """
    ...
    delta_arr = np.atleast_1d(np.asarray(delta_arr, dtype=float))
    n_params = H0_arr.shape[0]

    z_grid = _get_integration_grid(z_obs, z_grid_points)  # (nz,)

    H_grid = _solve_H_newton(
        z_grid[None, :],
        H0_arr[:, None], Om_arr[:, None], delta_arr[:, None],
    )  # (n_params, nz)
...
def chi2_diag(params, z_vals, mu_vals, inv_var):
    ...
    if not _within_bounds(params):
        return 1e12
    mu_th = mu_model(z_vals, H0, Om, delta)
...
def chi2_grid_diag(params_grid, z_vals, mu_vals, inv_var):
    """
    Chi-squared over a grid of (H0, Om, delta) using diagonal errors.
    ...
    delta_arr = valid_params[:, 2]

    mu_th = mu_model_batch(z_vals, H0_arr, Om_arr, delta_arr)  # (n_valid, n_z)
...
def model_mu_curvefit(z_array, H0, Om, delta):
    """curve_fit-friendly signature."""
    mu = mu_model(z_array, H0, Om, delta)
    if np.any(~np.isfinite(mu)):
...
def plot_chi2_profile_delta(best_x, chi2_best, z_vals, mu_vals, inv_var,
                            n_points=PROFILE_POINTS, outdir='.'):
    """1D profile chi^2(delta): H0 and Om re-fit at every delta (diagonal)."""
    H0_fit, Om_fit, delta_fit = best_x
    span = max(abs(delta_fit) * 1.5, 0.6)
    ...
    for i, d in enumerate(tqdm(deltas, desc="  Delta profile")):
        def chi2_reduced(p2):
            return chi2_diag([p2[0], p2[1], d], z_vals, mu_vals, inv_var)
    ...
    for level, label in zip(CONF_LEVELS_1D, [r'1$\sigma$', r'2$\sigma$', r'3$\sigma$']):
        ...
        ax.text(deltas[-1], level, label, va='bottom', ha='right', fontsize=9, color='gray')
    ax.set_xlabel(r'$\delta$')
    ax.set_ylabel(r'$\Delta\chi^2(\delta)$')
    ax.set_title(r'Profile likelihood: $\Delta\chi^2$ vs $\delta$ (Pantheon+ SNe)')
    ax.set_ylim(0, 12)
    ax.legend(fontsize=9)
    fig.tight_layout()
    fig.savefig(os.path.join(outdir, 'chi2_profile_delta_sn.png'), dpi=300, bbox_inches='tight')
    plt.close(fig)
...
def _param_range(center, bounds, rel_span=0.6, min_abs_span=0.3):
    ...
    The original code used [center*0.4, center*2.0], which only makes sense
    for center > 0 (as is the case for H0, Om). For a parameter like delta
    that can be negative, that formula can produce lo > hi. This instead
    builds a span around center that works regardless of sign, and falls
    back to the full bounds if the result is degenerate.
    """
...
def plot_confidence_ellipses_H0_Om(best_x, chi2_best, z_vals, mu_vals, inv_var,
    ...
    """H0-Om confidence contours at the best-fit delta."""
    H0_fit, Om_fit, delta_fit = best_x
    ...
    ax.set_title(rf'Confidence contours: $H_0$ vs $\Omega_{{m,0}}$ at $\delta={delta_fit:.3f}$ (SNe)')
    ax.legend()
...
def plot_hubble_diagram_clean(best_x, z_vals, mu_vals, mu_err, outdir='.'):
    ...
    mu_smooth = mu_model(z_smooth, H0_fit, Om_fit, delta_fit)
    mu_at_data = mu_model(z_vals, H0_fit, Om_fit, delta_fit)
    ...
    ax1.set_title('Hubble diagram (distance modulus): best-fit delta-LCDM')
...
def create_model_comparison_table(best_x, chi2_best, z_vals, mu_vals, cov_inv,
    ...
    """AIC / AICc / BIC comparison of delta-LCDM vs pure LambdaCDM."""
    print("\n  Fitting LambdaCDM for comparison...")
    bounds_lcdm = [BOUNDS[0], BOUNDS[1]]
    chi2_lcdm_fn = (lambda p: chi2_lcdm_cov(p, z_vals, mu_vals, cov_inv)) if use_full_cov else \
                   (lambda p: chi2_diag([p[0], p[1], 0.0], z_vals, mu_vals, inv_var))

    try:
        de_lcdm = differential_evolution(
            chi2_lcdm_fn, bounds=bounds_lcdm,
            seed=42, maxiter=150, tol=1e-7, polish=True, popsize=15,
        )
        H0_l, Om_l = de_lcdm.x
        chi2_lcdm_best = de_lcdm.fun
        res = minimize(chi2_lcdm_fn, [H0_l, Om_l], method='Nelder-Mead',
                       bounds=bounds_lcdm,
                       options={'xatol': 1e-7, 'fatol': 1e-7, 'maxiter': 4000})
        ...
        if use_full_cov:
            ...
            chi2_lcdm_best = chi2_diag([H0_l, Om_l, 0.0], z_vals, mu_vals, inv_var)
    ...
    with open(fname, 'w') as f:
        ...
        daic = aic_m - aic_l
        if daic < -2:
            f.write("delta-LCDM preferred by AIC\n")
        elif daic < 2:
...
def consistency_check_delta_zero(best_x, z_vals, mu_vals, inv_var):
    """Sanity check that delta=0 recovers LambdaCDM chi^2."""
    H0_fit, Om_fit, delta_fit = best_x
    print("\nBehaviour of chi^2 as delta -> 0 (H0, Om fixed at best fit):")
    for frac in [1.0, 0.5, 0.2, 0.1, 0.0]:
        d = delta_fit * frac
        c = chi2_diag([H0_fit, Om_fit, d], z_vals, mu_vals, inv_var)
        print(f"  delta={d:<8.4f} chi^2={c:.3f}")
    c_lcdm = chi2_diag([H0_fit, Om_fit, 0.0], z_vals, mu_vals, inv_var)
    print(f"  [cross-check] direct LambdaCDM (delta=0) chi^2: {c_lcdm:.3f}")


def export_best_fit_data(z_vals, mu_vals, mu_err, best_x, outdir='.'):
    H0_fit, Om_fit, delta_fit = best_x
    mu_best = mu_model(z_vals, H0_fit, Om_fit, delta_fit)
    residuals = mu_vals - mu_best

    z_smooth = np.linspace(max(z_vals.min() * 0.5, 1e-4), z_vals.max() * 1.05, 250)
    mu_smooth = mu_model(z_smooth, H0_fit, Om_fit, delta_fit)

    data_fname = os.path.join(outdir, 'delta_lcdm_sn_fit_results.txt')
    ...
    curve_fname = os.path.join(outdir, 'delta_lcdm_sn_smooth_curve.txt')
...
def write_fit_summary(best_x, perr, chi2_best, dof, flat_samples, sampler, outdir="."):
    ...
    with open(filename, "w") as f:
        f.write("===== BEST FIT (Pantheon+SH0ES, delta-LCDM) =====\n\n")
...
def main():
    ...
    outdir = os.path.join(script_dir, "results_delta_sn")
    ...
    try:
        ...
        # optionally adopt the curve_fit point if it improved chi2
        if use_full_cov:
    ...
    fig_corner.savefig(os.path.join(outdir, 'corner_delta_lcdm_sn.png'),
                       dpi=300, bbox_inches='tight')
    plt.close(fig_corner)

    # --- Profile & contours (use diagonal for speed) ---
    print("\n--- Profile likelihood & confidence contours for delta ---")
    # Recompute a diagonal chi2_best for consistent Delta-chi2 on the grid
    chi2_best_diag = chi2_diag(best_x, z_vals, mu_vals, inv_var)
    plot_chi2_profile_delta(best_x, chi2_best_diag, z_vals, mu_vals, inv_var, outdir=outdir)
    ...
    print("  Saved: chi2_profile_delta_sn.png, contour_delta_Om_sn.png, contour_delta_H0_sn.png")

    print("\n--- Confidence ellipses at best delta ---")
    H0_mesh, Om_mesh, dchi2 = plot_confidence_ellipses_H0_Om(
        best_x, chi2_best_diag, z_vals, mu_vals, inv_var, outdir=outdir
    )
    np.save(os.path.join(outdir, 'contour_H0_Om_delta_sn.npy'),
    ...
    consistency_check_delta_zero(best_x, z_vals, mu_vals, inv_var)
