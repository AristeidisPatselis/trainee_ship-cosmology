...
DATA_DIR = os.environ.get(
    'DELTA4_DATA_DIR',
    '/home/aristeidismp/Desktop/Aristeidis_Michailis_Patselis/Academia/Patra-Physics/Traineeship/Codes_0/Data_Sets/'
)
Z_FILE = 'c_z_vals.txt'
H_FILE = 'c_H_vals.txt'
...
def main():
    ...
    if Om_hi - Om_lo < 0.05:
        ...
        Om_hi = min(BOUNDS[1][1], Om_fit + 0.05)
    ...
    # NaN (not chi2()'s 1e12 sentinel) marks invalid cells: contourf/contour
    # in the comparison script treat NaN as a genuine masked gap, whereas a
    # finite-but-huge value like 1e12 gets linearly interpolated against its
    # valid neighbors, which can bite small notches out of the outer (3-sigma)
    # contour edge right where the ODE solver happened to fail.
    chi2_vals = np.full((n_grid, n_grid), np.nan)
    print(f"  Computing {n_grid}x{n_grid} H0-Om grid for contour saving...")
    for i in range(n_grid):
        for j in range(n_grid):
            params = (H0_mesh[i, j], Om_mesh[i, j], alpha_fit)
            if not _within_bounds(params):
                continue
