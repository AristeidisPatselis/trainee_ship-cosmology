...
Only the background (H(z)) data are used here (cosmic chronometers, same
c_z_vals.txt / c_H_vals.txt / c_sigma_vals.txt as delta_lcdm_fit.py) --
...
def plot_hubble_diagram_all_models(results, z_vals, H_vals, sigma_vals, lcdm_fit, outdir='.'):
    ...
    ax1.errorbar(z_vals, H_vals, yerr=sigma_vals, fmt='o', color='k',
                 ms=4, capsize=2, label='Cosmic chronometer data', zorder=5)
...
def main():
    ...
    script_dir = os.path.dirname(os.path.realpath(__file__))
    outdir = os.path.join(script_dir, "results_omega_fT")
    os.makedirs(outdir, exist_ok=True)
